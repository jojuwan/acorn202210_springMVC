...
// ExceptioinController 에서 처리되지 않은 예외가 발생하면 여기로 실행순서가 온다.
@Controller
public class ErrorHandler {
	// application.properties 에 server.error.path=/error 설정때문에 호출되는 컨트롤러 메소드
	@GetMapping("/error")
	public String handleError(HttpServletRequest request) {
		//에러의 코드값을 읽어온다 404, 500, 403 emdemd
		int code=(int)request.getAttribute(RequestDispatcher.ERROR_STATUS_CODE);
		
		//에러의 코드값 종류에 따라 선택적으로 응답할수 있다.
